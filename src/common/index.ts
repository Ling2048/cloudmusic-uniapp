...
const getSystemInfoInstance = () => {
  if (!SystemInfoInstance) {
    SystemInfoInstance = uni.getSystemInfoSync() as any;// getSystemInfoSync()
  }
...
const getCompatibleTop = () => {
  const systemInfo = getSystemInfoInstance()!;
  var e = getMenuButtonBoundingClientRectInstance().top || systemInfo.statusBarHeight
  return "Android 4.4.2" === systemInfo.system ? e = 8 : "iOS 9.0.1" === systemInfo.system && (e = 24), e;
}
...
export {
  getCount, parseRelativeDate, 
  getCapsule, 
  getCompatibleTop, getCompatibleWindowHeight,
  getCompatibleWindowWidth, 
  mergeSongBytrack, handleLyric, getLyricMap
}